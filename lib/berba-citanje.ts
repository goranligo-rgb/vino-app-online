/**
 * BERBA — CITANJE, jedno mjesto.
 *
 * Izvuceno iz `GET /api/berba` da izvoz u Excel (`POST /api/berba/export`)
 * cita TOCNO isto sto i stranica. Dva citanja istog izvora razisla bi se prvom
 * izmjenom jednoga — a izvoz obecaje "ono sto je na ekranu".
 *
 * Obrazlozenja (sto je zapis, zasto ULAZ, zasto tri upita) stoje u ruti
 * `app/api/berba/route.ts` i u komentarima nize.
 */

import { prisma } from "@/lib/prisma";

export async function citajBerbe() {
...
/** Jedan zapis kako ga cita server (datumi su `Date`). */
export type BerbaZapis = Awaited<ReturnType<typeof citajBerbe>>[number];
